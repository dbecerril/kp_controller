...
class dynamicsTab(QWidget):
    ...
    def _on_start(self):
        ...
        # Disable button + simple feedback via button text
        self.btn_start.setEnabled(False)
        self.btn_start.setText("Capturing…")
        ...
        # Wire signals (no progress hookup)
        self._thread.started.connect(self._worker.run)
        self._worker.finished.connect(self._on_finished)
        self._worker.error.connect(self._on_error)
        # Ensure the thread stops also on error
        self._worker.finished.connect(self._thread.quit)
    ...
    def _on_finished(self, t, y, label: str):
        ...
        self.btn_start.setText("Start Capture (sync light ON)")

    def _on_error(self, message: str):
        # Minimal error feedback in button text; log/alert elsewhere if you like
        self.btn_start.setEnabled(True)
        self.btn_start.setText("Start Capture (sync light ON)")
        # Optional: print or raise a signal if you want to surface this in Main
        print(f"[Dynamics] Error: {message}")
